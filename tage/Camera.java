...
public class Camera
{
	...
	/** Rotates the camera around its V (upward-facing) axis by the specified angle. */
	public void yaw(float angle) {
		Matrix4f rotation = new Matrix4f().rotate(angle, v);
		rotation.transformDirection(u);
		rotation.transformDirection(n);
	}

	/** Rotates the camera around its U (right-facing) axis by the specified angle. */
	public void pitch(float angle) {
		Matrix4f rotation = new Matrix4f().rotate(angle, u);
		rotation.transformDirection(v);
		rotation.transformDirection(n);
	}

	/** Rotates the camera around its N (forward-facing) axis by the specified angle. */
	public void roll(float angle) {
		Matrix4f rotation = new Matrix4f().rotate(angle, n);
		rotation.transformDirection(u);
		rotation.transformDirection(v);
	}

	/** Pans the camera horizontally and vertically relative to its current location */
	public void pan(float horizontalInc, float verticalInc) {
		Vector3f newLocation = new Vector3f(u).mul(horizontalInc).add(new Vector3f(v).mul(verticalInc));
		location.add(newLocation);
	}

	public void zoom(float zoomInc) {
		Vector3f newLocation = new Vector3f(n).mul(zoomInc);
		location.add(newLocation);
	}
